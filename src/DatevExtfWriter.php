...
class DatevExtfWriter
{
     ...
     * Constructor
     *
     * @param string $dataType Type of data (e.g., 'Buchungsstapel', 'Debitoren/Kreditoren')
     */
    public function __construct(string $dataType = 'Buchungsstapel')
    {
    ...
    {
        // Set format name based on data type
        $this->header->setFormatName($this->dataType);

        // Set appropriate format category based on data type
        switch ($this->dataType) {
            case 'Buchungsstapel':
                $this->header->setFormatCategory(21);
                $this->header->setFormatVersion(13);
                break;
            case 'Debitoren/Kreditoren':
                $this->header->setFormatCategory(16);
                $this->header->setFormatVersion(5);
                break;
            case 'Wiederkehrende Buchungen':
                $this->header->setFormatCategory(20);
                $this->header->setFormatVersion(2);
                break;
            case 'Kontenbeschriftungen':
                $this->header->setFormatCategory(65);
                $this->header->setFormatVersion(2);
                break;
            case 'Zahlungsbedingungen':
                $this->header->setFormatCategory(46);
                $this->header->setFormatVersion(2);
                break;
            case 'Diverse Adressen':
                $this->header->setFormatCategory(48);
                $this->header->setFormatVersion(4);
                break;
            default:
                throw new \InvalidArgumentException("Unsupported data type: $this->dataType");
        }
    ...
    {
        // Type checking based on data type
        if ($this->dataType === 'Buchungsstapel' && !($entry instanceof DatevExtfWriterEntry)) {
            throw new \InvalidArgumentException('For Buchungsstapel, entries must be DatevExtfWriterEntry objects');
        }

        if ($this->dataType === 'Debitoren/Kreditoren' && !($entry instanceof DatevExtfWriterParty)) {
    ...
    {
        // Get the column headers based on data type
        if ($this->dataType === 'Buchungsstapel') {
        ...
        } elseif ($this->dataType === 'Debitoren/Kreditoren') {
